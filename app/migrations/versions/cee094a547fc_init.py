...
Revision ID: cee094a547fc
Revises: 
Create Date: 2024-04-08 15:46:36.266410
...
def upgrade() -> None:
    ...
    sa.Enum('CHECKED_IN', 'CHECKED_OUT', name='eventtype').create(op.get_bind())
    ...
    sa.Column('event_date', sa.Date(), server_default=sa.text('now()'), nullable=False),
    sa.Column('event_type', postgresql.ENUM('CHECKED_IN', 'CHECKED_OUT', name='eventtype', create_type=False), nullable=False),
...
def downgrade() -> None:
    ...
    sa.Enum('CHECKED_IN', 'CHECKED_OUT', name='eventtype').drop(op.get_bind())
    # ### end Alembic commands ###
