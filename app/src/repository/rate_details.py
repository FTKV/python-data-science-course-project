...
from src.schemas.rate_details import RateDetailInput, RateDetailUpdate
...
class RateDetailRepository:
    ...
    async def get_rate_detail_by_id(self, rate_detail_id: int) -> RateDetailModel:
        result = await self.session.execute(
            select(RateDetailModel).filter(RateDetailModel.id == rate_detail_id)
        )
    ...
    async def update_rate_detail(
        self, rate_detail_id: int, rate_detail_update: RateDetailUpdate
    ) -> RateDetailModel:
